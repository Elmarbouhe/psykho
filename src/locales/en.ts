export const en = {
    common: {
        loading: 'Loading...',
        save: 'Save',
        cancel: 'Cancel',
        signOut: 'Sign Out',
        welcome: 'Welcome back,',
    },
    sidebar: {
        dashboard: 'Dashboard',
        dialogue: 'Dialogue',
        checkIn: 'Check-in',
        insights: 'Insights',
        profile: 'Profile',
        appName: 'Psykho',
    },
    profile: {
        title: 'Your Profile',
        description: 'Provide some information to help your AI companion understand you better.',
        fullName: 'Full Name',
        age: 'Age',
        occupation: 'Occupation',
        currentGoal: 'Current Goal',
        safeSpace: 'My Safe Space',
        saveProfile: 'Save Profile',
        saving: 'Saving Changes...',
        success: 'Profile updated successfully!',
        error: 'Failed to update profile',
        placeholders: {
            name: 'John Doe',
            age: '25',
            occupation: 'Software Engineer, Designer...',
            currentGoal: 'Reduce anxiety, sleep better...',
            safeSpace: "Describe a place that makes you feel peaceful (e.g., 'the beach at sunset')...",
        },
        language: 'Language',
    },
    chat: {
        title: 'Bana Companion',
        subtitle: "I'm here to listen, reflect, and support you. This is a safe space.",
        question: 'How are you feeling today?',
        thinking: 'Psykho is thinking...',
        inputPlaceholder: 'Type a message...',
        disclaimer: 'AI-generated content may be inaccurate.',
    },
    auth: {
        loginTitle: 'Welcome to the Sanctuary',
        loginSubtitle: 'A safe space for your mind to unwind and reflect.',
        registerTitle: 'Join the Sanctuary',
        registerSubtitle: 'Create an account to start your journey of reflection.',
        emailLabel: 'Email Address',
        passwordLabel: 'Password',
        loginButton: 'Continue with Email',
        registerButton: 'Sign Up',
        googleButton: 'Continue with Google',
        loggingIn: 'Logging in...',
        creatingAccount: 'Creating account...',
        noAccount: "Don't have an account?",
        hasAccount: 'Already have an account?',
        signUpLink: 'Sign up',
        loginLink: 'Login',
        backToLogin: 'Back to Login',
        or: 'OR',
        emailPlaceholder: 'name@example.com',
        passwordPlaceholder: '••••••••',
        googleAlt: 'Google',
    },
};
